...
#   2. Clones the repo to /opt/nook and runs npm install + frontend build
#   3. Interactively creates .env
#   4. (Optional) Installs Caddy as a reverse proxy with auto-HTTPS
#   5. (Optional) Installs a systemd service for auto-start
# ──────────────────────────────────────────────────────────────
...
if [[ -f "$ENV_FILE" ]]; then
  ...
  if ! ask_yes_no "Overwrite existing .env?"; then
...
if [[ ! -f "$ENV_FILE" ]]; then
  ...
  [[ -z "$LLM_KEY" ]] && err "LLM API key is required"

  echo ""
  AGENT_PW=$(ask_secret "Agent web UI password")
  [[ -z "$AGENT_PW" ]] && err "Password is required"
...
echo ""
info "Phase 4: Reverse Proxy"

if ask_yes_no "Install Caddy as a reverse proxy? (handles HTTPS automatically)" "y"; then
  ...
  DOMAIN=$(ask_value "Domain name (or leave blank for :80)" "")
  ...
  if [[ -n "$DOMAIN" ]]; then
    ...
    info "Caddy configured for :80 (HTTP only)"
  fi
  ...
  info "Skipping Caddy — access the agent directly on port 3001"
fi
...
if ask_yes_no "Install systemd service? (auto-start on boot)" "y"; then
  ...
  info "Skipping systemd — start manually with: cd $INSTALL_DIR && node index.js"
fi
...
if [[ -n "${DOMAIN:-}" ]]; then
  ok "Access your agent at: https://$DOMAIN"
elif command -v caddy &>/dev/null && systemctl is-active caddy &>/dev/null; then
  ok "Access your agent at: http://$(hostname -I | awk '{print $1}')"
else
  ok "Access your agent at: http://$(hostname -I | awk '{print $1}'):3001"
fi
